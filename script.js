...
const PLACEHOLDER_VACANT = "PuzzleLeagueFiles/Sprites/vacantSquare/"
...
let music =  mySound = new Audio(`PuzzleLeagueFiles/Audio/Music/${MUSIC[randInt(MUSIC.length)]}`)
...
const ANNOUNCER_CHAIN_ARRAY = ["beautiful.wav", "incredible I can't believe it.wav", "you don't see moves like that everyday folks.wav", 
                             ...
                             "we are never going to forget this event.wav","unbelievable.wav"]
const ANNOUNCER_TIME_TRANSITION = ["time marches on.wav"]
const ANNOUNCER_HURRY_UP = ["ten seconds to destiny.wav", "it all boils down to these last moments.wav"]
...
if (localStorage.getItem("highScore") === null) {localStorage.setItem("highScore", "1000")}
...
class Block extends Position {
    constructor(x,y,color,type, delay = 0, touched = false, 
        availableForPrimaryChain = false, availableForSecondaryChain = false) {
        super(x,y)
        this.color = color
        this.type = type
        this.delay = delay
    ...
    draw(ctx) {
        ...
        if (this.type == CLEARING) {
            ...
            } else if (this.delay > 5 || this.delay<0) {
                BLOCK_IMAGE.src = this.color + this.type + "0.png"
...
function playChainSFX(currentChain) {
    ...
    if (currentChain < 9) {
        mySound = new Audio(`PuzzleLeagueFiles/audio/Super Mario 64 Red Coin ${currentChain-1}.wav`)
    } else {
        mySound = new Audio(`PuzzleLeagueFiles/audio/Super Mario 64 Red Coin 8.wav`)
    }
    mySound.volume = .1
    mySound.play();
}
...
function extractTimeToIndex(dateTimeAPI) {
    if (dateTimeAPI.length == 0) {return console.log("JSON not fetched yet")}
    let index = 0;
...
function playMusic(file, volume = .2, mute=0) {
    music.play()
    music.volume = volume
    if (mute==1) {music.volume = 0}
    else {music.volume = .2}
    mute = (mute+1)%2
}
...
function isChainActive(board) {
    ...
    if (chain>7) {playSFX(`Announcer/${ANNOUNCER_CHAIN_ARRAY[7]}`), volume = .3}
    else if (chain>1) {playSFX(`Announcer/${ANNOUNCER_CHAIN_ARRAY[chain-2]}`), volume = .3}
        ...
        if (legalMatch(board, clearLocations)) {
            ...
            if (chain==0){
                ...
                if (clearLocationsLength>3) {
                    playSFX(`Announcer/${ANNOUNCER_COMBO_ARRAY[randInt(ANNOUNCER_COMBO_ARRAY.length)]}`)
                } else {
                    // playChainSFX(chain)
        ...
        for (let c=0; c<COLS; c++) {
            if (i==0) {
                ...
                if (board[c][2].color != VACANT) {
                    playSFX(`Announcer/${ANNOUNCER_PANIC[randInt(ANNOUNCER_PANIC.length)]}`)
                    break
                }
            }
        }
...
function gameOverBoard(board) {
    ...
    if (frames==1) {music.pause(); music.currentTime=0; playSFX("topout.wav")}
    disableRaise = true
...
function updateScore(clearLocationsLength, currentChain) {
    ...
    if (score>highScore){highScore = score; highScoreDisplay.style.color = "gold"}
}
...
function CONTROL(event){
    if (running) {
        ...
        if (event.keyCode == 13 && dateTimeAPI.length != 0) { // enter
            database = data[0]
    ...
    if (running & !gameOver){
        ...
        if (event.keyCode >= 0 && frames>=200) { //any key
            gameOver = false
            board = makeOpeningBoard(randInt(1440))
        }
    }
...
function gameLoop(timestamp) {
    frames++
    if (!running) {return}
    if (frames==2) {window.scrollTo(0,document.body.scrollHeight);}
    ...
    if (frames==6600 && !gameOver) {playSFX(`Announcer/ten seconds to destiny.wav`)}
    if (frames%1200==0 && level<10 && level>0 && developerEnabled==0 && !gameOver) { // Speed the stack up every 30 seconds
        if (frames == 3600) {
            ...
            playSFX(`Announcer/${ANNOUNCER_OVERTIME[randInt(ANNOUNCER_OVERTIME.length)]}`)
        }
